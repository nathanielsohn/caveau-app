...
3. **Cross-tier boundaries in answers.** When a Collector member asks about a feature that's Estate-only (e.g., Private Location Monitoring, bonded courier), should the advisor describe it plainly (it informs an upgrade conversation) or deflect ("that's not part of your current tier")? Recommend: describe it plainly, treat the advisor as the member's trusted guide to the full Caveau platform.
