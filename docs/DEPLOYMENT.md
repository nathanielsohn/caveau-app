...
| `CERTIFICATE_HMAC_SECRET` | No | Independent HMAC key for provenance certificate hashes (`src/lib/certificate-hash.ts`). Falls back to `NEXTAUTH_SECRET` if unset — fine for the demo; set a dedicated random value in real production so a leak of one secret doesn't compromise the other. |
