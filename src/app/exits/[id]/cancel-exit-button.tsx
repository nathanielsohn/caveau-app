...
import { cancelExitFacilitation } from "../actions";

export default function CancelExitButton({ exitId }: { exitId: string }) {
  ...
  return (
    <button
      ...
      onClick={() => {
        if (
          !confirm(
            "Cancel this consignment request? You can submit a new one any time.",
          )
        ) {
          return;
        }
        startTransition(async () => {
          const res = await cancelExitFacilitation(exitId);
          if (res.ok) {
            router.refresh();
          } else {
