...
| 48 | ~~Private Location Monitoring (Year 2)~~ | ~~Shipped — `Facility.type` enum (`vault` vs `private_location`) with owner scoping, certification fields (`privateLocationCertifiedAt`, `locationInstallerId`), `privateLocationKind`, and a `LocationInstaller` model. Members manage owned private locations from `/settings/locations`; `/facility` renders a dedicated private-location view when the active location is member-owned (certification badge, installation partner contact, device roster + link to /sentinel). Admin surfaces: `/admin/installers` roster + activation toggle, and `/admin/facilities/[id]` Private Location Monitoring panel for assigning installers and marking a private location certified. Admin Overview includes a live “Certified locations” metric. Seed ships a certified “Saenz Residence Wine Room” with a monitor locker + SensorPush-style sensor device.~~ |
...
| 52 | ~~Concierge migration~~ | ~~48-hour white-glove import from CellarTracker and Vivino CSV exports. Column-mapping UI, admin-facing migration queue for staff fulfillment. Positioned as the churn-killer on slides 5, 10, 17. Done.~~ |
...
| 55 | ~~Exit signals~~ | ~~AI-surfaced sell-window alerts per bottle, tied to Liv-ex momentum + drink-window intersection. Visible on dashboard, wine detail, AI Advisor. Pipeline from signal → #47 exit facilitation. Slides 5, 6. Done. `ExitSignal` model added in migration 0034 with a partial unique index ensuring at most one open signal per wine. Scoring pass in `src/lib/exit-signals.ts` opens signals on drink-window proximity (within 5 years of end) or 12-month price momentum (≥ +12%), upgrades to "strong" when either leg trips hard, and closes stale signals automatically. Dashboard teaser card, wine detail badge + panel with handoff CTA, AI Advisor `getExitSignals` tool. Seed runs the reconciliation pass against Robert's portfolio so the demo state reflects live rules.~~ |
| 56 | ~~Insurance savings estimate~~ | ~~Dashboard / portfolio card: collection value × tier storage discipline → estimated 20–35% premium savings range with PURE / Chubb / AXA XL / Berkley One named. Static math, no carrier API required. Slide 9. Done. `src/lib/insurance.ts` runs the math (1.0–1.5% baseline premium × tier-scaled 15–35% discount band); `InsuranceSavingsCard` renders on dashboard (below exit signals) and on `/portfolio` (above the bottles table). AI Advisor `getInsuranceSavingsEstimate` tool answers slide 6 Q4 ("how much am I saving on insurance?") with the same math as the UI.~~ |
| 57 | ~~Portfolio vs. Liv-ex 100~~ | ~~YTD portfolio performance charted against the Liv-ex Fine Wine 100 index. Slide 5 dashboard tile; slide 6 canonical advisor question. Done. `src/lib/portfolio-timeseries.ts` reconstructs month-end portfolio value from `WineValuation` history and indexes both series to 100 at the YTD anchor (trailing-12-month fallback when YTD has fewer than 3 Liv-ex snapshots). Bottles added after the anchor are excluded so the baseline is stable. Dashboard teaser card and `/portfolio` full chart render the three-stat delta; AI Advisor `getPortfolioVsLivex` tool returns both indexed series + a ready-made delta so slide-6 Q2 resolves in one tool call instead of two plus manual math.~~ |
| 58 | ~~Sentinel fleet / device admin~~ | ~~Device-level registry per facility/locker: firmware version, battery %, WiFi vs. LTE-M connectivity state, last heartbeat, alert routing. Surfaces the "proprietary IoT hardware with patent portfolio" story from slide 17. Done. `SentinelDevice` + `SentinelDeviceEvent` models added in migration 0035 with a nullable `SensorReading.deviceId` FK. Single table carries static inventory + mutable heartbeat state; `model` enum distinguishes locker sensors from Bottle Probes (Estate-tier accessory with nullable `wineId` FK). `bundledWithTier` records which tier entitled a device; `tiers.ts` gained `bundledSentinels` + `bundledBottleProbes` counts per tier. Admin fleet list at `/admin/sentinels` with facility + connectivity + "needs attention" filters; detail at `/admin/sentinels/[id]` with assignment card, recent readings, event log, and staff actions (reassign, mark firmware updated, test ping, retire). Ingest route at `/api/ingest/sensor` resolves `deviceSignature` → device, writes `sensor_readings.device_id`, upserts mutable status, and emits events on firmware/battery/connectivity transitions. Seed runs 9 devices across Naples + Miami covering WiFi/LTE-M/offline/pool/retired/low-battery/stale-firmware states plus a Bottle Probe paired with Robert's Pétrus.~~ |
| 59 | ~~Sentinel inventory & tier assignment~~ | ~~At-signup device allocation per tier (Collector purchase, Reserve 1 included, Private Vault 2 included, Estate 2 + Bottle Probe). Serial capture, location assignment, activation state. Slides 8, 12. Done. Onboarding wizard grew a fourth step (tier → locker → devices → first bottle); for bundle tiers it reads `bundledSentinels`/`bundledBottleProbes` off `tierSpec`, consumes the oldest pool units at the member's facility, and transactionally flips them to installed in step 2's reserved locker (with a demo-alive heartbeat shortcut: `connectivity=wifi`, `batteryPct=100`, `lastHeartbeatAt=now`). Pool shortage soft-fails with "N shipping this week" copy. Collector variant is a pure-copy upsell panel with no server action. `/settings` gets a "Your Sentinel devices" card — read-only per-device status (connectivity, battery, last heartbeat) plus a member-scoped `pairBottleProbeAction` that lets Estate members pair their probe with an in-cellar wine without elevating to admin. Naples seed pool bumped to 6 locker sensors + 2 Bottle Probes + 1 Miami unit so live demos can absorb multiple back-to-back signups.~~ |
| 60 | ~~Allocation access~~ | ~~Private Allocations feed: staff posts limited releases with per-tier eligibility; members request; staff fulfills. Founding benefit on slide 11; $1.5K–$5K annual value quoted on slide 9. Done. `Allocation` + `AllocationRequest` models added in migration 0036 with a nullable `Wine.sourceAllocationId` back-link so fulfilled requests carry provenance into the member's collection. Eligibility math lives in `src/lib/allocations.ts` (single source of truth for feed filter, detail guard, request submit, advisor tool) and combines `minimumTier` (ordered via new `TIER_RANK` helper), `foundingOnly`, and `foundingEarlyAccess` rules. Two-stage lifecycle — `submitted → accepted → fulfilled` with `declined` / `cancelled` branches — so staff can reserve bottles before payment (#27) lands; fulfillment writes one Wine row per bottle transactionally. Member feed at `/allocations` with eligibility-gated cards + teaser preview for unauthenticated visitors; detail at `/allocations/[slug]` with request form capped at 3 bottles per member per release. Admin at `/admin/allocations` mirrors the migrations status-chip pattern, per-allocation roster with accept/decline/fulfill actions, and CSV export. Publish fires a best-effort `notifyEligibleOnPublish` via SES. AI Advisor `getMyAllocations` tool answers "what can I request?" in one call. Seed ships three releases: DRC 2021 (Estate + founding early), Opus One 2021 (Private-Vault-and-up with Robert's pending request), and Screaming Eagle 2020 (fulfilled, with bottle in Robert's collection).~~ |
| 61 | ~~Welcome appraisal~~ | ~~Point-in-time valuation document distinct from the Caveau Custody & Condition Report — basis, date, appraiser, purpose, heirs (estate-scoped). Extends founding-member onboarding. Revenue stream on slide 15 (Appraisal & Estate Docs, $5K–$15K Y1). Done. `Appraisal` model added in migration 0037 with three enums (`AppraisalStatus`, `AppraisalBasis`, `AppraisalPurpose`) and a two-stage request → complete lifecycle. `src/lib/appraisals.ts` owns tier-scaled pricing ($495/$795/$1,195/$1,595), founding welcome eligibility, portfolio line-item snapshot, and `CAV-APR-YYYY-NNNN` number minting. Document integrity via HMAC-SHA256 in `src/lib/appraisal-hash.ts` (reuses `CERTIFICATE_HMAC_SECRET` — same trust model as the CCR). Member surfaces: `/appraisals` list, `/appraisals/new` request form with purpose + basis + optional scoped picker + dynamic heirs fieldset (estate-only), `/appraisals/[id]` detail that renders the full document when completed. Admin queue at `/admin/appraisals` with status chips + CSV export; completion action re-snapshots at commit time and writes `line_items` JSON so the document stays stable after issuance. Public `/verify/appraisal/[hash]` verifies without exposing line items. Server-rendered PDF via `pdf-lib` (`src/lib/appraisal-pdf.ts`) at both `/api/appraisals/[id]/pdf` (member) and `/admin/appraisals/[id]/pdf` (staff). AI Advisor `getMyAppraisals` tool returns status, verify URL, and `welcomeState` (available / claimed / ineligible) so "is my appraisal ready?" and "can I still claim my free one?" resolve in one call. Dashboard nudge + `/settings` Founding Circle row surface the CTA; Robert (founding Estate) ships with CAV-APR-2026-0001 completed covering his full portfolio.~~ |
| 62 | ~~Acquisition sourcing~~ | ~~Member-requested bottle sourcing via Liv-ex or Caveau's private network. Request form → admin queue → fulfillment record → 8–12% margin tracked. Revenue stream #8 on slides 12 and 15. Done. `Acquisition` model added in migration 0038 with `AcquisitionStatus` (requested / sourcing / fulfilled / declined / cancelled) and `AcquisitionSource` (livex / broker / auction / caveau_private) enums, plus a nullable `Wine.sourceAcquisitionId` back-link mirroring the #60 allocation pattern. Single-stage lifecycle — the member-accept step that would normally sit between `sourcing` and `fulfilled` is deferred until Stripe (#27) can gate it behind actual payment; `estimated_total_usd` carries the staff-to-member quote communication in the interim. Margin math in `src/lib/acquisitions.ts` (`computeMargin` computes mark-up-on-price to match the 8–12% slide-15 band; admin UI + CSV export flag out-of-band rows). Member surfaces at `/acquisitions` (list), `/acquisitions/new` (structured form with exact-or-range vintage, optional region/varietal/budget/notes), `/acquisitions/[id]` (detail with member-visible fields only — no cost or margin leaked). Admin surfaces at `/admin/acquisitions` (status-chip queue + CSV export with margin columns), `/admin/acquisitions/[id]` (lifecycle actions + fulfill form with live margin preview + budget-cap warning). Fulfillment transactionally writes `quantity` Wine rows into the member's collection with `sourceAcquisitionId` set. AI Advisor `getMyAcquisitions` tool returns status / spec / quote / member price / sourced wine IDs but deliberately omits cost and margin (admin-only). Seed: Robert gets one `sourcing` request (2010 Lafite, $2,800 quote against $3K cap) and one `fulfilled` (2019 Sassicaia, 3 bottles, 9.7% margin, all back-linked into his collection).~~ |
