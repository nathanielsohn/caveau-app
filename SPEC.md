...
| 48 | ~~Home Cellar Program (Year 2)~~ | ~~Shipped — `Facility.type` enum (`vault` vs `home_cellar`) with certification fields (`homeCellarCertifiedAt`, `homeCellarInstallerId`) and a new `HomeCellarInstaller` model (migration 0045). Member `/facility` renders a dedicated Home Cellar Program view when the active facility is a home cellar (certification badge, installer contact, device roster + link to /sentinel). Admin surfaces: `/admin/installers` roster + activation toggle, and `/admin/facilities/[id]` Home Cellar Program panel for assigning installers and marking a home cellar certified. Admin Overview includes a live “Certified cellars” metric. Seed ships a certified “Saenz Home Cellar” with a monitor locker + SensorPush-style sensor device.~~ |
