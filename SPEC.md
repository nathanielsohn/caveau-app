...
| 51 | Biometric-verified Deliver Now _(in progress)_ | Vault → member home delivery flow. App-side ladder: biometric re-auth (Face ID/Touch ID) → delivery PIN → address confirmation → step-up OTP for deliveries >$2K. Door-side ladder: government-ID scan → name-match-to-account → authorized recipient registry → photo + timestamp log. Meets Florida DABT requirements. Slide 7. **Status (2026-04-17):** data model, member ladder, door-side ladder, and wine-detail entry point landed across `f46fd01`, `1bb372a`, `9285075`, `5303875`. Remaining: OTP step-up for >$2K deliveries, ID-match completion flow, end-to-end verification. |
